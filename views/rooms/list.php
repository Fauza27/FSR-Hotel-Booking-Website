            ...
            <div class="rooms-grid">
                <?php foreach ($rooms as $room): ?>
                    <div class="room-card">
                        <div class="room-img">
                            <?php 
                            // Pastikan $room->primary_image_url ada dan tidak kosong
                            // dan juga pastikan metode di Room.php yang mengisi $rooms sudah menyertakan 'primary_image_url'
                            // seperti yang telah kita bahas di perbaikan sebelumnya (getAllRooms, getRoomsByCategory, searchAvailableRooms).
                            ?>
                            <?php if (!empty($room->primary_image_url)): ?>
                                <img src="<?= APP_URL . '/assets/images/rooms/' . htmlspecialchars($room->primary_image_url); ?>" alt="<?= htmlspecialchars($room->room_number); ?>">
                            <?php else: ?>
                                <!-- Opsi jika Anda ingin ruang kosong jika tidak ada gambar -->
                                <!-- <div class="image-placeholder-empty" style="height: 200px; background: #eee; display: flex; align-items: center; justify-content: center; color: #aaa;">No Image</div> -->
                                <!-- Atau tidak menampilkan elemen gambar sama sekali jika tidak ada gambar -->
                            <?php endif; ?>
